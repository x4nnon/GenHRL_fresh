...
def load_cfg_from_registry(task_name: str, entry_point_key: str) -> Union[dict, object]:
    ...
    # Try to get the spec, handling gymnasium version parsing issues
    print(f"🔧 DEBUG: Looking for task_id: '{task_id}'")
    try:
        spec = gym.spec(task_id)
        print(f"🔧 DEBUG: Found spec directly for {task_id}")
    except gymnasium.error.NameNotFound as e:
        print(f"🔧 DEBUG: NameNotFound for {task_id}: {e}")
        # If not found, try adding -v0 suffix for G1_generated environments
        if "Isaac-G1" in task_id and not task_id.endswith("-v0"):
            try:
                spec = gym.spec(f"{task_id}-v0")
                print(f"🔧 DEBUG: Auto-resolved {task_id} to {task_id}-v0")
            except gymnasium.error.NameNotFound:
                raise gymnasium.error.NameNotFound(f"Environment `{task_id}` doesn't exist (also tried {task_id}-v0)")
        else:
            raise
    ...
    if isinstance(cfg_entry_point, str) and cfg_entry_point.endswith(".yaml"):
        if os.path.exists(cfg_entry_point):
            ...
            if mod.__file__ is None:
                raise ValueError(f"Could not determine path for module: {mod_name}")
...
def parse_env_cfg(
    task_name: str, device: str = "cuda:0", num_envs: Union[int, None] = None, use_fabric: Union[bool, None] = None
) -> Union[ManagerBasedRLEnvCfg, DirectRLEnvCfg]:
    ...
    # simulation device
    cfg.sim.device = device  # type: ignore
    # disable fabric to read/write through USD
    if use_fabric is not None:
        cfg.sim.use_fabric = use_fabric  # type: ignore
    # number of environments
    if num_envs is not None:
        cfg.scene.num_envs = num_envs  # type: ignore

    return cfg


def get_checkpoint_path(
    log_path: str, run_dir: str = ".*", checkpoint: str = ".*", other_dirs: Union[list[str], None] = None, sort_alpha: bool = True
) -> str:
