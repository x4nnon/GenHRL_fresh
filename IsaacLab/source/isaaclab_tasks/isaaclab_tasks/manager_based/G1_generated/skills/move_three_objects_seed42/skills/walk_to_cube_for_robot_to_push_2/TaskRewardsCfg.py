...
def walk_to_cube_for_robot_to_push_2_main_reward(env: ManagerBasedRLEnv, normalise: bool = True, normaliser_name: str = "main_reward") -> torch.Tensor:
    """
    Main reward for the robot to walk to a position near the second 'Cube for robot to push' (Object2)
    to prepare for pushing. The goal state is for the robot's pelvis to be positioned appropriately
    to initiate a push on Object2.
    """
    ...
    # Object2 dimensions (0.5m cubed block) - Hardcoded from object configuration as per rules
    object2_size_x = 0.5

    # Calculate target x position relative to Object2 for pushing (behind the cube)
    # Robot should be at object2.x - (object2_size_x / 2 + buffer_distance)
    ...
    target_x_offset = object2_size_x / 2 + 0.25 # 0.25 + 0.25 = 0.5m behind center
    target_pelvis_x = object2.data.root_pos_w[:, 0] - target_x_offset # Relative distance calculation
    ...
    distance_y = torch.abs(pelvis_pos_y - object2.data.root_pos_w[:, 1])
...
def walk_to_cube_for_robot_to_push_2_collision_avoidance_reward(env: ManagerBasedRLEnv, normalise: bool = True, normaliser_name: str = "collision_avoidance_reward") -> torch.Tensor:
    """
    ...
    especially with the target cube (Object2) before the push.
    """
    # Get normalizer instance
    RewNormalizer = get_normalizer(env.device)
...
def walk_to_cube_for_robot_to_push_2_forward_progress_stability_reward(env: ManagerBasedRLEnv, normalise: bool = True, normaliser_name: str = "forward_progress_stability_reward") -> torch.Tensor:
    """
    This reward encourages the robot to make consistent forward progress towards Object2 (in the x-direction)
    while maintaining an upright and stable posture (pelvis z-height). It also penalizes excessive lateral
    movement (y-direction) that is not contributing to alignment with Object2.
    """
    # Get normalizer instance
    RewNormalizer = get_normalizer(env.device)
    ...
    # Object2 dimensions (0.5m cubed block) - Hardcoded from object configuration
    object2_size_x = 0.5

    # Target x for pushing is object2.x - 0.5 (from primary reward)
    # This is the ideal x-position for the pelvis relative to the cube for pushing.
    target_pelvis_x_for_push = object2.data.root_pos_w[:, 0] - (object2_size_x / 2 + 0.25)
    ...
    x_progress_reward = torch.where(
        ...
        torch.exp(-2.0 * torch.abs(pelvis_pos_x - target_pelvis_x_for_push)), # Reward for approaching from behind
        -2.0 * torch.abs(pelvis_pos_x - target_pelvis_x_for_push) # Penalty for overshooting or being too far past
    )

    # Reward for minimizing lateral deviation from Object2's y-coordinate.
    # This encourages the robot to align itself centrally with the cube.
    # This is a relative distance reward.
    lateral_deviation_y = torch.abs(pelvis_pos_y - object2.data.root_pos_w[:, 1])
    ...
    pelvis_z_target = 0.7
    pelvis_z_stability_reward = torch.exp(-5.0 * torch.abs(pelvis_pos_z - pelvis_z_target))
...
class TaskRewardsCfg:
    # Main reward for positioning the robot near Object2 for pushing.
    # Weight 1.0 indicates this is the primary objective.
    MainWalkToCubeForPushReward = RewTerm(func=walk_to_cube_for_robot_to_push_2_main_reward, weight=1.0,
                                          params={"normalise": True, "normaliser_name": "main_reward"})

    # Shaping reward for collision avoidance with all objects.
    # Weight 0.4 provides a significant penalty for collisions without overshadowing the main goal.
    CollisionAvoidanceReward = RewTerm(func=walk_to_cube_for_robot_to_push_2_collision_avoidance_reward, weight=0.6,
                                       params={"normalise": True, "normaliser_name": "collision_avoidance_reward"})

    # Shaping reward for encouraging forward progress and maintaining stability.
    # Weight 0.3 guides the robot's movement and posture during the approach.
    ForwardProgressStabilityReward = RewTerm(func=walk_to_cube_for_robot_to_push_2_forward_progress_stability_reward, weight=0.1,
