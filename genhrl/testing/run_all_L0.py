...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Print or run commands to train all L0 skills across multiple seeds",
    )
    ...
    parser.add_argument("--steps", type=int, default=20000, help="Steps per run (default: 10000)")
    parser.add_argument("--num_envs", type=int, default=4096, help="Number of environments (default: 4096)")
    parser.add_argument(
        "--seeds",
        type=int,
        nargs="+",
        default=[123, 456, 789, 101112],
        help="List of seed values (default: 4 seeds)",
    )
    ...
    if not args.run:
        print(f"\nTraining commands for L0 skills in task '{args.task_name}' across seeds {args.seeds}:")
        if args.skip_first > 0:
    ...
    for skill in l0_skills:
        ...
        for seed in args.seeds:
            cmd = orch.build_simple_training_command(
                skill_name=skill,
                steps=args.steps,
                ...
                seed=seed,
                use_random_policies=False,  # not relevant for L0
            )
            if not args.run:
                ...
                if proc.returncode != 0:
                    print(f"Run failed for {skill} (seed={seed}) with exit code {proc.returncode}")
                # Cleanup newly created runs if requested
                if args.no_save:
                    after = _snapshot_top_level_runs(logs_dir)
                    removed = _cleanup_new_runs(before, after)
                    if removed > 0:
                        print(f"[CLEANUP] Removed {removed} new log run(s) for {skill} (seed={seed})")
...
# example usage
# python3 testing/run_all_L0.py --task_name obstacle_course_seed42 --no_save --no_success_saving --run