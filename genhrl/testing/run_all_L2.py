...
def list_l2_skills(orchestrator) -> list:
    """Return all level-2 composite skills for the task in orchestrator."""
    order = orchestrator.get_training_order()
    return [
        s
        for s in order
        if (not orchestrator.is_skill_primitive(s)) and orchestrator.get_skill_level(s) == 2
    ]
...
def _build_flat_l2_command(
    ...
    """Build a flat PPO command for an L2 composite skill using IsaacLab's skrl/train.py.
    ...
    env_id = (
        f"Isaac-RobotComposite{gym_suffix}-{flat_suffix}-v0"
        if flat_suffix
        else f"Isaac-RobotComposite{gym_suffix}-v0"
    )

    # Flat training uses IsaacLab train.py and --max_iterations
    # Align with L0/L1 logic: assume rollouts=24 to approximate iterations from steps
    rollouts = 24
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Print or run commands to train all L2 skills (hierarchical and flat) across multiple seeds",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=[111, 222, 888, 999, 101010],
        help="Seed values for hierarchical L2 runs (default: 5 seeds)",
    )
    parser.add_argument(
        "--seeds_flat",
        type=int,
    ...
    parser.add_argument("--skip_first", type=int, default=0, help="Skip the first N L2 skills before running")
    parser.add_argument("--run_baseline", default=True, action="store_true", help="Run baseline PPO for all L2 skills")
    ...
    if not l2_skills:
        # Gracefully skip if no L2 skills for this task
        print(f"No L2 skills found for task: {args.task_name}. Skipping.")
        return 0
    ...
    if not args.run:
        print(f"\nTraining commands for L2 skills in task '{args.task_name}':")
        if args.skip_first > 0:
            print(f"(Skipping first {args.skip_first} L2 skills)")
        print("=" * 80)
    ...
    for skill in l2_skills:
        if not args.run:
            print(f"\n# Skill: {skill} [L2]")
        # Hierarchical HPPO runs via orchestrator (train_l2.py)
        for seed in args.seeds_l2:
            ...
            cmd_flat = _build_flat_l2_command(
...
"""
example usage
python3 testing/run_all_L2.py --task_name obstacle_course_seed42 --no_save --no_success_saving --run
"""
