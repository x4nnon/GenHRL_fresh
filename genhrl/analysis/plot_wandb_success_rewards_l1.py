...
def _plot_skill_series_dual(skill_name: str,
    ...
    out_path = output_dir / f"{skill_name}_l1_hppo_vs_flat_success_reward.png"
    try:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Plot W&B success reward for all L1 skills (GenHRL+HPPO vs Flat baseline)",
    )
    ...
    for skill in l1_skills:
        ...
        computed_flat = _compute_wandb_project_for_l1_flat(args.task_name, skill, args.flat_suffix)
        ...
        if collected_hppo:
            # Use the oldest runs (last runs) instead of the most recent
            collected_hppo.sort(key=lambda t: t[0])  # ascending: oldest first
        ...
        if collected_flat:
            # Use the oldest runs (last runs) instead of the most recent
            collected_flat.sort(key=lambda t: t[0])  # ascending: oldest first
...
# python3 genhrl/analysis/plot_wandb_success_rewards_l1.py \
