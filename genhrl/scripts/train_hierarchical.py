...
"""
Script to train hierarchical RL agent with skrl.
This is an extension of the standard train.py script that integrates with 
the hierarchical PPO implementation.
"""
...
# Import the hierarchical PPO implementation
from genhrl.hierarchical_ppo.hppo import register_hierarchical_ppo
...
parser = argparse.ArgumentParser(description="Train a hierarchical RL agent with skrl.")
parser.add_argument("--video", action="store_true", default=False, help="Record videos during training.")
parser.add_argument("--video_length", type=int, default=200, help="Length of the recorded video (in steps).")
parser.add_argument("--video_interval", type=int, default=2000, help="Interval between video recordings (in steps).")
parser.add_argument("--num_envs", type=int, default=4096, help="Number of environments to simulate.")
parser.add_argument("--task", type=str, default="Isaac-G1FlatPick_Up_Nearest_Object_With_Both_Hands-v0", help="Name of the task.")
...
# Hierarchical PPO specific arguments
parser.add_argument("--algorithm", type=str, default="hierarchicalppo", help="The RL algorithm to use (should be hierarchicalppo).")
parser.add_argument("--skill_name", type=str, required=True, help="Name of the skill being trained.")
parser.add_argument("--policy_type", type=str, default="primitive", choices=["primitive", "option", "meta"], 
                    help="Type of policy in the hierarchy (primitive, option, meta).")
parser.add_argument("--subpolicies", type=str, default="", 
                    help="Comma-separated list of subpolicy names used by this skill.")
parser.add_argument("--repository_path", type=str, default="policy_repo", 
                    help="Path to the policy repository for saving/loading policies.")
parser.add_argument("--skills_path", type=str, default=None,
                    help="Path to the skills directory for saving policies.")
...
# Register hierarchical PPO with skrl
register_hierarchical_ppo()
...
agent_cfg_entry_point = "skrl_cfg_entry_point" if algorithm in ["ppo", "hierarchicalppo"] else f"skrl_{algorithm}_cfg_entry_point"


@hydra_task_config(args_cli.task, agent_cfg_entry_point)
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    """Train with hierarchical skrl agent."""
    ...
    log_root_path = os.path.join("logs", "skrl", args_cli.skill_name.lower())
    ...
    if "wandb_kwargs" in agent_cfg["agent"]["experiment"]:
        ...
        # Use skill name for project name
    ...
    if isinstance(env.unwrapped, DirectMARLEnv) and algorithm in ["ppo", "hierarchicalppo"]:
    ...
    # Add hierarchical PPO configuration
    agent_cfg["agent"]["skill_info"] = {
        "name": args_cli.skill_name,
        "type": args_cli.policy_type,
        "subpolicies": args_cli.subpolicies.split(",") if args_cli.subpolicies else [],
        "repository_path": args_cli.repository_path,
        "skills_path": args_cli.skills_path
    }
